...
- For this offline snapshot, the current latest released version is `0.3.305`.
...
- Good: restore the exact-RID self-contained package selected by the resolver at `0.3.305`.
- Good: in explicit .NET CLI mode, restore all three core packages at `0.3.305`.
...
```powershell
dotnet add package Techne.Loom.Abstractions --version 0.3.305
dotnet add package Techne.Loom.Common --version 0.3.305
dotnet add package Techne.Loom.SkillOrchestrator --version 0.3.305
```

Self-contained acquisition uses the one exact runtime package selected after RID detection:

```text
https://www.nuget.org/api/v2/package/Techne.Loom.SkillOrchestrator.Runtime.<rid>/0.3.305
```
