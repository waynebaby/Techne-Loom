...
- Good .NET CLI path: restore the three IL packages above at `0.3.291-beta`, validate the host/CLI preflight, then use one unified runtime directory.
- Good self-contained path: restore exactly one `Techne.Loom.SkillOrchestrator.Runtime.<rid>` package at `0.3.291-beta`, validate its hash and manifest, then use its direct executable.
...
```powershell
dotnet add package Techne.Loom.SkillOrchestrator --version 0.3.291-beta
...
```text
Techne.Loom.SkillOrchestrator.Runtime.<rid> @ 0.3.291-beta
```
