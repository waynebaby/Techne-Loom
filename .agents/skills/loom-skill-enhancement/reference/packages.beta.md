...
- For this offline snapshot, the current latest beta version is `0.3.318-beta`.
...
- Good .NET CLI path: restore the three IL packages above at `0.3.318-beta`, validate the host/CLI preflight, then use one unified runtime directory.
- Good self-contained path: restore exactly one `Techne.Loom.SkillOrchestrator.Runtime.<rid>` package at `0.3.318-beta`, validate its hash and manifest, then use its direct executable.
...
```powershell
dotnet add package Techne.Loom.SkillOrchestrator --version 0.3.318-beta
dotnet add package Techne.Loom.Common --version 0.3.318-beta
dotnet add package Techne.Loom.Abstractions --version 0.3.318-beta
```

Self-contained fallback acquisition uses one exact package after RID detection:

```text
Techne.Loom.SkillOrchestrator.Runtime.<rid> @ 0.3.318-beta
```
...
- `resolved_runtime_version: 0.3.318-beta`
