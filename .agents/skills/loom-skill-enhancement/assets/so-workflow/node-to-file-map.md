...
- Bound SO package runtime version: `0.3.320-beta`.
...
| `transition.reacquire_runtime` | host-native acquisition of the exact locked SO product+RID package; records source/hash/package identity, safe archive validation, extraction path, apphost path, and package readiness evidence |
...
| `transition.capture_guide` | first direct `so.exe --guide` (Windows) or `so --guide` (Unix) call from the verified extraction; returned guide paths become the version-specific authority |
