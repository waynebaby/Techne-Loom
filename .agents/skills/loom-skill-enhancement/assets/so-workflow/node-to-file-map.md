...
- Bound SO package runtime version: `0.3.319-beta`.
...
| `transition.start_mcp` | shared governance-entry check: use `assets/agents/loom-skill-enhancement-mcp-startup.agent.md` with the resolver-owned launch descriptor to select CLI or confirmed MCP by host capability, inspect a bounded fragment, and persist `mcp_startup_evidence` before guide capture |
