...
- Current published AO package runtime version: `0.3.291-beta`.
...
- After every AO CLI call or audit-producing step, including `dotnet ao.dll` and self-contained `ao.exe` (or `ao`) calls, follow [Mermaid artifact delivery](reference/mermaid-artifact-delivery.md): read the actual `audit_artifacts.mermaid_file` and `audit_artifacts.html_file`, verify existence and readability, and never guess an audit path. For `workspace_mirror` with `link_resolvable=true`, use only the hash-verified `workspace_relative_mermaid_file` and `workspace_relative_html_file` values for Markdown links. Pass the verified `card_input_file` or absolute Mermaid and HTML paths only to host card or notification actions. For `runtime_path_only`, retain absolute paths as technical evidence or host action targets, not Markdown links. If the current call has no `mermaid_delivery` object, the host may derive `not_emitted` and reuse only a previously verified workspace-relative link while stating that the render is unchanged. On `delivery_failed`, report the failure and do not emit a link, notification, or reuse an earlier link.
