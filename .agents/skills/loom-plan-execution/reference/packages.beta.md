...
- For this offline snapshot, the current latest beta version is `0.2.183-beta`.
...
- Good: restore all three packages at `0.2.183-beta`.
- Bad: restore one package at an older prerelease while another package uses the current bound version `0.2.183-beta`.
...
```powershell
dotnet add package Techne.Loom.Abstractions --version 0.2.183-beta
dotnet add package Techne.Loom.Common --version 0.2.183-beta
dotnet add package Techne.Loom.AgentOrchestrator --version 0.2.183-beta
```
...
- `resolved_runtime_version: 0.2.183-beta`
