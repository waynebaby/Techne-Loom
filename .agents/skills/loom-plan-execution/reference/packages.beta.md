...
- For this offline snapshot, the current latest beta version is `0.3.320-beta`.
...
- Good framework path: restore the three IL packages above at `0.3.320-beta`, validate the host/CLI preflight, then use one unified runtime directory.
- Good self-contained path: restore exactly one `Techne.Loom.AgentOrchestrator.Runtime.<rid>` package at `0.3.320-beta`, validate its hash and manifest, then use its direct executable.
...
```powershell
dotnet add package Techne.Loom.AgentOrchestrator --version 0.3.320-beta
dotnet add package Techne.Loom.Common --version 0.3.320-beta
dotnet add package Techne.Loom.Abstractions --version 0.3.320-beta
```

Self-contained fallback acquisition uses one exact package after RID detection:

```text
Techne.Loom.AgentOrchestrator.Runtime.<rid> @ 0.3.320-beta
```
...
- `resolved_runtime_version: 0.3.320-beta`
