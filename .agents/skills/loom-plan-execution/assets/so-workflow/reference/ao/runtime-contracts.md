...
- AO compile also emits `workflow.compile-feedback.json` using the shared `workflow.compile-feedback.v1` contract. Validation and parse failures retain the workflow backup and feedback without placeholder Mermaid or HTML files.
- Valid workflow, template, schema, demo, runtime-copy, audit-backup, and compile-feedback JSON files written to disk use indented multi-line formatting; compact JSON remains limited to JSONL and protocol payloads.
- Output paths are normalized real filesystem paths and remain valid when outside the Git worktree or ignored. Verify existence and readability before reporting them; when `--workspace-root` is supplied, use the verified workspace-relative mirror for direct editor opening.
