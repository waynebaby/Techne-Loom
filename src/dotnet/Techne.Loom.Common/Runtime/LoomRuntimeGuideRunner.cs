...
public static class LoomRuntimeGuideRunner
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var launch = LoomRuntimeLaunch.CreateCommand(identity, ["--guide"]);
        ...
        if (!process.Started)
        {
            throw new LoomRuntimeHostStartupException($"Self-contained apphost '{launch.Command}' could not start for the guide operation.");
        }

        if (process.ExitCode != 0)
        {
            throw new LoomRuntimeCommandException($"Runtime '--guide' failed after apphost startup with exit code {process.ExitCode}: {FirstNonEmpty(process.StandardError, process.StandardOutput)}");
        }
        ...
        try
        {
            ...
            if (!string.Equals(LoomRuntimeCatalog.NormalizeVersion(version), identity.Version, StringComparison.Ordinal)
            ...
            {
                throw new LoomRuntimeGuideValidationException("The guide result did not match the apphost version or documentation root.");
            }

            var expectedRelativePath = $"guides/{LoomRuntimeCatalog.GetEntryPoint(identity.Product)}-guide.md";
            ...
            {
                throw new LoomRuntimeGuideValidationException($"The guide result returned '{relativePath}', expected '{expectedRelativePath}'.");
            }
        ...
        {
            throw new LoomRuntimeGuideValidationException("The guide result was missing or could not be read.", exception);
        }
    }
