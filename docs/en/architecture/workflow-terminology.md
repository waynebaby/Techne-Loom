...
## Skill Roles / Skill 角色

- **Enhancing skill / 执行增强的 skill**：`/loom-skill-enhancement`，负责创建、升级或治理另一个 skill。
- **Skill being enhanced / 被增强的 skill**：本次由 enhancing skill 创建或修改的 skill。它不是产品名的一部分。
- **Skill under Loom Skill Orchestrator governance / 受 Loom Skill Orchestrator 治理的 skill**：已经拥有受治理 workflow 的 skill。可以在不含歧义时简称 `skill under Loom Skill Orchestrator governance`。
- **Loom Agent Plan-Execution Orchestrator**：AO 的用户侧产品名；保留 `Techne.Loom.AgentOrchestrator`、直接 `ao`/`ao.exe` apphost 命令、`ao-guide.md` 和 `/loom-plan-execution` 等实现身份。
...
- Use emoji as a stable second meaning channel: `🧭` intake, `🔎` research, `💬` review, `📝` drafting, `✅` completion, `⚙️` runtime, `📜` contract, `🧾` evidence, `❓` decision, `🚧` blocked, and `🔁` continuation.
- Use a color family that reinforces the emoji meaning, add a nearby legend, and keep color from being the only meaning channel.
- Chinese diagrams put English first and Chinese on the next line. Bilingual labels and HTML breaks are quoted.
- A workflow JSON or `WorkflowInstance` example must also show the Mermaid generated by the same-version `so` or `ao compile`; a hand-authored diagram is explanatory context, not compile evidence.
...
- [Loom Agent Plan-Execution Orchestrator Guide](../guides/ao-guide.md)
- [SkillOrchestrator Guide](../guides/so-guide.md)

## Human-Friendly Status Mapping / 人类友好状态映射

This table is the single source of truth for human-facing wording across AO skills, SO skills, `so-*` skills, and skills under Loom Skill Orchestrator governance. Keep the exact internal token only in machine-readable contracts, source code, logs, audit evidence, and other implementation-facing surfaces; use the human-friendly wording in user-facing status, explanations, errors, and questions.

这张表是 AO skill、SO skill、`so-*` skill 以及受 Loom Skill Orchestrator 治理的 skill 的用户可见表达唯一真理源。精确的内部字面值只能保留在机器可读契约、源代码、日志、审计证据和其他实现侧表面；用户可见的状态、说明、错误和提问必须使用面向人的表达。
...
The table applies equally to AO, SO, `so-*`, and every skill under Loom Skill Orchestrator governance. The same file is intentionally bilingual so English and Chinese wording cannot drift into separate authorities.

这张表同样适用于 AO、SO、`so-*` 以及所有受 Loom Skill Orchestrator 治理的 skill。文件特意采用中英合一的形式，避免英文和中文措辞漂移成两套独立规范。

## Plain-Language Feedback For Every Language / 所有语言的普通人反馈

This glossary is for people maintaining the product. It is not a script for talking to users. Every user-facing update from AO, SO, or a skill under Loom Skill Orchestrator governance must use the user's requested language and ordinary words. English is not automatically plain language.
...
这份术语表用于产品维护，不是让用户照着学习后才能得到反馈。AO、SO 和所有受 Loom Skill Orchestrator 治理的 skill 的用户可见更新，都必须使用用户指定的语言和日常词汇；英文也不天然等于简单语言。
...
| Weave back / 织回 | An external participant returns structured data that re-enters the same strand and allows resume.<br/>外部参与方返回结构化数据，重新进入同一条推进线并允许恢复。 | `ao.exe resume`, `so.exe resume`, result envelopes<br/>`ao.exe resume`、`so.exe resume`、result envelope |
