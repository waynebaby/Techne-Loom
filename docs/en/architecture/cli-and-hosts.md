...
- AO apphost commands (`ao.exe` on Windows, `ao` on Unix) use disk-backed workflow files; canonical `--workflow-file` plan, replan, run, resume, and status paths are sessionless, while the older `session_dir + session_id` path remains a compatibility adapter.
...
- A blocked `<so_property>` payload is SO's current weave-out surface, and direct `so.exe resume --result-file` / `so resume --result-file` is the weave-back entry point.
