...
- in exclusive Loom Skill Orchestrator governance mode, direct CLI remains a primitive path, while local stdio MCP is the mandatory first external interface after runtime preflight for bounded verification; neither MCP calls nor CLI snippets replace official skill runs
...
- when exclusive Loom Skill Orchestrator governance mode applies, declare that Loom Skill Orchestrator is the only official execution authority, that only `dotnet so.dll run` / `resume` count as official skill runs, and that local stdio MCP must start first and be used for bounded verification without becoming a peer official run surface
