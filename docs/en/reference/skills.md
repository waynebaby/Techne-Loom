...
- store the workflow template separately and place runtime artifacts under `<target-skill-root>/assets/so-workflow/audit/`
...
- released/beta package index link set, including localized mirrors when they exist
- guide surface references
- deterministic workflow template path produced by planner flow, after guide-alignment review plus the SO workflow-file load/status gate succeed; that validated template becomes the execution authority for the enhanced target skill
- runtime return payload links, including audit artifacts

### Runtime handoff

- uses `dotnet so.dll --guide [--lang <language>]` as the source of truth
- lets the AI agent execute `dotnet so.dll planner` / `run` / `resume` directly in the terminal
- uses `dotnet so.dll planner` to materialize workflow JSON from a fine-grained description file that is planner input only
- validates that the resulting workflow template is complete and detailed against the selected channel guide, and also confirms it passes the current public SO workflow-file load/status gate before treating it as the execution authority; the current public CLI does not expose a standalone `compile` command, and a successful `dotnet so.dll status --workflow-file <path>` confirms load/status compatibility rather than workflow completeness by itself
