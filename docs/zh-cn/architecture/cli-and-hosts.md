...
- AO apphost 命令（Windows 为 `ao.exe`，Unix 为 `ao`）使用落盘 workflow 文件；canonical `--workflow-file` plan、replan、run、resume 和 status 路径是 sessionless 的，旧 `session_dir + session_id` 路径则作为兼容适配层保留。
...
- 被阻塞的 `<so_property>` payload 是 SO 当前的 weave-out surface；direct `so.exe resume --result-file` / `so resume --result-file` 是 weave-back 入口。
