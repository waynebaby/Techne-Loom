...
`[regex]::Match(...)` 是 PowerShell 语法，不是 C#。SO expression 必须使用 C# 形式：
...
[返回 SO 参考索引](so-guide-reference.md) | [English version](../../en/guides/so-guide-reference-tools.md)
