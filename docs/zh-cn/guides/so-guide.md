...
SkillOrchestrator 执行确定性的 workflow 步骤，只有在 workflow 完成或到达需要外部参与的 seam 时才返回。它是受 Loom Skill Orchestrator 治理的 skill 的执行权威。
...
完整的治理流程、被增强 skill 的规则、契约、示例和反模式位于 [SO Guide 完整参考](so-guide-reference.md)。hub 路径继续作为 `guide_path`，完整文档包会携带链接的 flow 与 reference 页面。