...
先阅读 [SO Flow](so-guide-flow.md) 了解治理执行路径；需要持久化数据结构时阅读 [Contracts](so-guide-reference-contracts.md)，需要确认职责时阅读 [Behavior And Responsibilities](so-guide-reference-behavior.md)，需要确定性执行规则时阅读 [Governance](so-guide-reference-governance.md)，最后阅读示例与反模式章节。

[返回 SO Guide Hub](so-guide.md)