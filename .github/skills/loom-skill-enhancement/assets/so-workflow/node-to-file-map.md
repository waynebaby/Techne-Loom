...
| `transition.analyze_scope` | `assets/so-workflow/skill-plan.md` |
...
| `transition.finalize_lock` | external completion manifest resolved under the OS temp root from `.tmp/loom-skill-enhancement-completion-manifest.md`, governing checked-in `SKILL.md`, `assets/so-workflow/so-package-lock.json`, `assets/so-workflow/governance-notes.md`, and `assets/so-workflow/node-to-file-map.md` |
