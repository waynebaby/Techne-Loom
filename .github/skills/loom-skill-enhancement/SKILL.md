...
10. Record that resolved package version in `<target-skill-root>/assets/so-workflow/so-package-lock.json` together with the chosen channel, package identity, and runtime bundle members.
...
16. When the enhanced target skill is used later, restore the full SO runtime bundle from the locked version in `so-package-lock.json`, resolving that exact bundle version from NuGet first and freshly downloading it unless the local cache already holds that exact version bundle, then clone the checked-in source template to an external runtime workflow copy before any `dotnet so.dll run` or `resume` call.
