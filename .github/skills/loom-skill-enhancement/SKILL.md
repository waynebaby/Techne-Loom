...
- when SO execution or target-skill restoration needs local NuGet acquisition, restore the SO runtime bundle instead of only the SO package: `Techne.Loom.SkillOrchestrator`, `Techne.Loom.Common`, and `Techne.Loom.Abstractions`, all at the same resolved channel/version
- when SO execution or target-skill restoration needs a local package runtime, resolve one exact SO bundle version first, acquire all three SO runtime-bundle packages in one pass, then extract them into one external unified runtime directory before running any `so.dll` command; do not probe or execute from partial single-package extraction roots
- when exact-version NuGet acquisition fails, reacquire that same three-package SO runtime bundle from GitHub fallback assets for the same resolved version, rebuild the same unified runtime directory, and only then continue with `so.dll`
...
- the enhanced target skill `SKILL.md` must explicitly reference `<target-skill-root>/assets/so-workflow/so-package-lock.json` as the authoritative SO runtime version lock, and it must say that day-to-day SO runtime restoration resolves the exact locked runtime bundle from NuGet first and freshly downloads it unless the local cache already holds that exact version bundle
- when the enhanced target skill is later used, restore the full SO runtime bundle from that locked version instead of silently floating to a newer version within the same channel or omitting `Common` / `Abstractions`
...
Think-out-loud output is incomplete unless it explicitly reports all of these runtime fields once the package runtime is prepared and again on every later SO progress update:

- `resolved_runtime_version`
- `runtime_bundle_packages`
- `unified_runtime_directory`

After every SO progress update, think-out-loud output must also explicitly report:
...
10. When local package-channel runtime execution is needed, acquire `Techne.Loom.SkillOrchestrator`, `Techne.Loom.Common`, and `Techne.Loom.Abstractions` together at that same version and extract them into one external unified runtime directory before any `so.dll` command.
11. If exact-version NuGet acquisition fails, reacquire that same three-package SO bundle from GitHub fallback assets for the same resolved version and rebuild the unified runtime directory instead of switching to one-off package probing.
12. Record that resolved package version in `<target-skill-root>/assets/so-workflow/so-package-lock.json` together with the chosen channel, package identity, and runtime bundle members.
13. Update the enhanced target skill `SKILL.md` so it explicitly references `<target-skill-root>/assets/so-workflow/so-package-lock.json` as the authoritative SO runtime version lock, and so it states that routine SO runtime bundle restoration resolves the exact locked bundle from NuGet first, reacquires all three bundle packages together, and runs only from one unified runtime directory unless the local cache already holds that exact version bundle.
14. Author or refresh the deterministic workflow JSON template under `<target-skill-root>/assets/so-workflow/` from the reviewed plan and supporting references.
15. Unless the user explicitly chooses another destination, point compile and audit temporary output to a runtime temporary root or repo-root temporary root, not to the target skill path or `assets/so-workflow/`.
16. Run `dotnet so.dll compile --workflow-file <path> [--audit-output <path>]`.
17. Validate that the workflow JSON template is complete and detailed against the selected channel guide, then require `dotnet so.dll compile` to succeed before treating that workflow template as the execution authority for the enhanced target skill.
18. When the enhanced target skill is used later, restore the full SO runtime bundle from the locked version in `so-package-lock.json`, resolving that exact bundle version from NuGet first and freshly downloading it unless the local cache already holds that exact version bundle, then rebuild the unified runtime directory before any `dotnet so.dll run` or `resume` call.
19. Keep that mutable runtime copy plus its `.events.jsonl` sidecar outside the target skill path unless the user explicitly chooses another execution output root.
20. Keep `--audit-output` outside the target skill path too; runtime workflow copies, event sidecars, and audit artifacts do not belong in the skill folder.
21. Run `dotnet so.dll run` / `resume` against that runtime copy. When variance appears, use `status` plus `inspect-workflow` / `inspect-events` to locate drift, then update the source workflow JSON through the same authoring flow and re-run `compile`.
22. Use the structured blocked payload such as `current_step_kind` to classify whether a weave-out is waiting for mandatory user input, waiting for external asynchronous results, or explicitly allowing non-human continuation, and then consume `skill_hint` literally as the next action instruction.
23. Every time SO reports progress for the enhanced target skill, render the current workflow to Mermaid Markdown and HTML under runtime temp or explicit execution-output roots, and force-report `resolved_runtime_version`, `runtime_bundle_packages`, `unified_runtime_directory`, `audit_markdown_file`, and `audit_html_file` in think-out-loud output.
...
- locked SO runtime bundle package list for target-skill restoration: `Techne.Loom.SkillOrchestrator`, `Techne.Loom.Common`, and `Techne.Loom.Abstractions`
- unified runtime directory path used for local SO CLI execution when package-channel runtime acquisition was used
...
- explicit note that day-to-day target-skill execution restores the locked SO runtime bundle from NuGet and freshly downloads it unless the local cache already holds that exact version bundle, while a new enhancement pass always resolves the latest bundle version from the user-chosen channel and freshly downloads it unless that exact version is already present in local cache, then rewrites the lock
- think-out-loud runtime fields on every SO progress update: `resolved_runtime_version`, `runtime_bundle_packages`, and `unified_runtime_directory`
- current workflow Mermaid Markdown and HTML paths on every SO progress update, surfaced as explicit `audit_markdown_file` and `audit_html_file` entries in think-out-loud output for the enhanced target skill
