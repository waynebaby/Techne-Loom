...
- Validate SHA-512, package identity, nuspec metadata, manifest, entrypoint, ZIP safety, and size bounds before launch. For NuGet.org, compare the downloaded package with `catalogEntry.packageHash` from the exact registration response; a missing public flat-container `.nupkg.sha512` is not itself a package failure. Require and verify `.nupkg.sha512` sidecars for GitHub exact fallback assets and local release artifacts. Isolate user-level cache entries by product, exact version, and RID; protect them with a cross-process lock; validate in a temporary directory; and publish atomically.
