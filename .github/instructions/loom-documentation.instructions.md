...
- AO-facing docs use `Loom Agent Execution Orchestrator` in titles, intros, README positioning, and guide navigation while preserving `ao-guide.md`, `dotnet ao.dll`, and package identifiers.
- Do not use legacy narrative labels such as `SO Governance`, `SO-enhanced`, or `SO-governed`. Prefer `Loom-governanced target skill`, `Loom Skill Orchestrator governance`, or the narrower status wording required by the current slice.
...
- User-facing progress, blocked, error, and completion updates for `/loom-plan-execution`, `/loom-skill-enhancement`, and Loom-governanced target skills use the user's requested language and plain words.
...
- When SO creates or updates a target skill, copy this language rule into target-skill user instructions, local subagent prompts, failure guidance, and workflow hints.
