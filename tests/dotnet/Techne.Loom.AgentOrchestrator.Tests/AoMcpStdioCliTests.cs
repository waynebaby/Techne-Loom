...
public sealed class AoMcpStdioCliTests
{
    [Fact]
    public async Task Cli_McpStdio_ExposesOnlyAoWorkflowTools()
    {
        ...
        Assert.DoesNotContain("<ao_property>", stdout, StringComparison.Ordinal);
        ...
        Assert.Equal("Loom Agent Execution Orchestrator", initialize.RootElement.GetProperty("result").GetProperty("serverInfo").GetProperty("name").GetString());
        ...
        Assert.All(names, name => Assert.StartsWith("ao_", name, StringComparison.Ordinal));
        Assert.Contains("ao_inspect_workflow_fragment", names);
    }
