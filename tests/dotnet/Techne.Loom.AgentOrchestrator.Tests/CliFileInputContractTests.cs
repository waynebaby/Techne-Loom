...
public sealed class CliFileInputContractTests
{
    ...
    public async Task WorkflowScript_RejectsInlineContentAndDoesNotCreateOutput()
    {
        ...
        var scriptFile = Path.Combine(Path.GetTempPath(), $"loom-ao-script-{Guid.NewGuid():N}.cs");
        var inputFile = Path.Combine(Path.GetTempPath(), $"loom-ao-input-{Guid.NewGuid():N}.json");
        var outputFile = Path.Combine(Path.GetTempPath(), $"loom-ao-output-{Guid.NewGuid():N}.json");
        ...
        await File.WriteAllTextAsync(inputFile, "{\"runtimeBinding\":\"dotnet-ao\"}");
    ...
    public async Task WorkflowScript_MissingReferenceFileFailsBeforeCandidateOutput()
    {
        ...
        var inputFile = Path.Combine(Path.GetTempPath(), $"loom-ao-input-{Guid.NewGuid():N}.json");
        var outputFile = Path.Combine(Path.GetTempPath(), $"loom-ao-output-{Guid.NewGuid():N}.json");
        var verificationFile = Path.Combine(Path.GetTempPath(), $"loom-ao-verification-{Guid.NewGuid():N}.json");
        var missingReference = Path.Combine(Path.GetTempPath(), $"loom-ao-reference-missing-{Guid.NewGuid():N}.json");
        await File.WriteAllTextAsync(scriptFile, "public static class S { public static WorkflowInstance Build(WorkflowScriptInput input) => new(); }");
        await File.WriteAllTextAsync(inputFile, "{\"runtimeBinding\":\"dotnet-ao\"}");
    ...
    {
        var root = FindRepositoryRoot();
        var targetFile = Path.Combine(Path.GetTempPath(), $"loom-ao-patch-target-{Guid.NewGuid():N}.txt");
