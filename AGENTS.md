...
- GitHub Copilot must not use the `apply_patch` tool in this repository. Use a checked-in range editor or another repository-approved file-editing mechanism instead.
- Prefer external-file-driven scripts for deterministic text and code edits and validation. Keep expected boundary text, replacement content, and validation inputs in separate files; do not embed multiline edit content in a command or script invocation.
- Use the checked-in range editor that matches an available runtime: `scripts/ApplyLineRangeEdit.csx` with `dotnet script`, `scripts/ApplyLineRangeEdit.js` with `node`, `scripts/ApplyLineRangeEdit.py` with `python`, `scripts/ApplyLineRangeEdit.ps1` with Windows PowerShell or PowerShell 7, or `scripts/ApplyLineRangeEdit.sh` with Bash. These five entries implement the same manifest contract documented in `scripts/README.md`.
- Prefer the runtime-native entry that is available in the current environment: PowerShell for native Windows orchestration and file edits, Bash for Git Bash/WSL/Linux, Node.js for JavaScript/TypeScript tooling, Python for Python tooling, and `.csx` when `dotnet script` is the selected repository tool.
- Every entry accepts the same `--target-file` and `--edits-file` options and the same external JSON manifest. The manifest uses 1-based inclusive ranges, external expected start/end boundary files, and an external replacement file. Paths are resolved from the process working directory.
- For one target file with multiple modifications, pass one manifest containing non-overlapping ranges. Each entry reads the target once, validates every original range and boundary, applies ranges from highest line number to lowest, writes through a same-directory temporary file, and validates the complete result before and after replacement.
- Range editors must preserve strict UTF-8, the target file's UTF-8 BOM, LF/CRLF/CR newline style, and trailing-newline state. Boundary comparison normalizes only leading/trailing whitespace and case. A failed pre-write validation must leave the target unchanged and return a non-zero exit code.
- Do not replace this contract with marker or general string replacement, inline multiline content, implicit wrapper nesting, skipped boundary checks, skipped overlap checks, or skipped post-write validation. If the selected runtime is unavailable, report the environment block rather than silently changing the edit semantics.
- Use PowerShell primarily for Windows command orchestration, environment inspection, `dotnet`, and Git invocation. Use Bash primarily for WSL/Linux-native restore, build, and test validation; use the Bash range editor for file edits only when Bash is the selected available environment and its dependencies are present.
- Use Node.js primarily for JavaScript/TypeScript tooling or JSON processing; if it edits source text, it must follow the same external file-driven and post-write validation contract.
...
- Runtime selection belongs to the platform-aware runtime resolver: before any package-cache lookup or network request, automatic mode probes for a usable `dotnet` host with `Microsoft.NETCore.App 9.x` or a higher major version. When available, the resolver selects framework-dependent DLL mode; otherwise it selects the exact-RID published self-contained executable. AO skills, SO skills, `so-*` skills, and Loom-governanced target skills provide only the exact bound runtime version; they must not bind or persist the OS, architecture, libc, RID, package id, executable name, cache directory, or launch path. Explicit `dotnet-cli` and self-contained selections are allowed, but the selected mode is immutable for that resolution and failures do not silently select the other mode.
- `.NET CLI mode` uses one exact-version .NET runtime bundle (a NuGet restore set) with a usable `Microsoft.NETCore.App 9.x` or higher-major host; the bundle must include the embedded Roslyn compiler assemblies used by the C# expression evaluator. Self-contained mode uses one exact-version RID package from the `Techne.Loom.AgentOrchestrator.Runtime.<rid>` or `Techne.Loom.SkillOrchestrator.Runtime.<rid>` family. A single resolution must acquire only the package closure for its locked mode: DLL/dependency/Roslyn packages or one RID package, never both. A failed resolution may switch modes only after it stops and receives explicit continuation under a new resolution identity.
...
- Automatic mode probes the local host before any package-cache lookup or network request. A usable `Microsoft.NETCore.App 9.x` or higher-major host selects framework-dependent mode; no usable host selects self-contained mode. Explicit mode selection is allowed and is resolved before any package acquisition.
- Framework-dependent mode validates and acquires only the exact-version DLL package closure, including dependency packages and the Roslyn compiler assemblies required by the C# expression evaluator. It must not request a RID Runtime EXE package.
- Self-contained mode validates and acquires only one exact-RID package for the selected product and platform: `Techne.Loom.AgentOrchestrator.Runtime.<rid>` for AO or `Techne.Loom.SkillOrchestrator.Runtime.<rid>` for SO. It must not request the DLL, dependency, or Roslyn package closure.
- Once a mode is selected, a failure stays in that mode and fails closed. Switching modes requires a new resolution identity and explicit continuation; it must never mix package closures in one resolution.
...
- The release-set manifest defines the active file/package scope, historical and test exclusions, channel rules, and validation policy. It is scope metadata, not a second version authority. In `check-in` mode, the selected CI/CD channel's NuGet flat-container metadata is the published-version baseline; in `release` mode, the single shared version job output is the candidate authority.
- During `check-in`, resolve all 20 package ids from the selected CI/CD channel on NuGet. Fail closed on missing or unreadable metadata, an unsupported channel version, or mixed versions. Only after all 20 packages resolve to one channel-compatible exact version may the workflow update the active release-set version surfaces; the manifest must not become a second current-version authority.
- During `release`, `PACKAGE_VERSION` comes only from the shared version job. A NuGet `latest` result must never replace or invalidate that candidate, because public feed indexing is eventually consistent.
...
- Active document-copy `source_sha256` values are SHA-256 hashes of canonical UTF-8 source text with CRLF/CR normalized to LF and trailing whitespace removed. Publish refresh scripts, release-set validation, and runtime checked-in asset validation must use this same form; a mismatch remains a hard failure.
...
- Before `Push packages`, CI must validate the local 20-package artifact closure against the shared candidate: exact package filenames, nuspec identity, exact versions for every internal Loom dependency, and runtime package metadata and guide versions. This local build validation is the package correctness gate and must not depend on NuGet indexing.
- After package push, CI must refresh all release-set-owned README, package-index, guide, skill, document-copy manifest, node-map, and package-lock surfaces, then validate those refreshed surfaces together with the same local package artifacts. A separate public-feed convergence probe may retry the exact candidate version, but it must not compare the candidate with a transient NuGet `latest` value or be the sole package correctness check.
...
- Whenever a Mermaid audit file is emitted or refreshed, user-facing output must first try a Mermaid card display tool with the verified absolute file path. If inline display is unavailable, use a clickable host notification whose open actions carry the verified absolute Mermaid and HTML paths. If the host has no clickable notification capability, emit Markdown links using verified workspace-relative paths when `link_resolvable=true`, and show the exact absolute paths as adjacent technical text. The current chat renderer does not reliably make Windows absolute filesystem paths clickable in Markdown, so an absolute path is not a substitute for a workspace-relative chat link.
