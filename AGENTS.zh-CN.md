...
- 这条规则适用于 `/loom-plan-execution`（AO）、`/loom-skill-enhancement`（SO）以及所有由 Loom 治理的 target skill 的用户可见进度、阻塞、错误和完成反馈。
- 使用用户指定的语言。无论是中文、英文还是其他语言，都要让不了解 workflow 软件的高中生看懂；英文不是天然的简单语言。
- 使用短句、常用词和直接的动词，按顺序说明：发生了什么、用户的工作或数据是否仍然安全、为什么会这样、下一步做什么。
- 不要让读者先理解软件是怎么构成的。第一层说明不要堆内部状态、步骤类型、节点 ID、检查名称、阶段名、交接术语、运行时术语或审计术语。必须使用技术词时，先在同一句话里用普通话解释它。
- 精确的命令、路径、ID、状态值和 evidence 字段，只有在帮助用户操作或核对结果时，才放到单独的“技术细节”小节。机器可读 payload 和审计记录可以保留精确的内部字面值。
- 遇到阻塞或失败，要说明是用户要做的工作有问题，还是工具、文件或输出目录阻止了继续；同时说明哪些结果仍然有效，并给出一个明确的下一步。
- SO 创建或更新 target skill 时，target skill 的用户说明、本地 subagent 提示、失败说明和 workflow hint 也必须遵守这条语言规则。
